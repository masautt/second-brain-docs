# Second Brain Docs

This documentation is organized by purpose:

## Start Here

- [Architecture Overview](./architecture/overview.md)
- [Micro-Frontend Composition](./architecture/mfe.md)
- [Data Pipeline](./architecture/data-pipeline.md)

## Apps

- [second-brain-web (shell)](./apps/second-brain-web.md)
- [finapp](./apps/finapp.md)
- [pinapp](./apps/pinapp.md)
- [future apps](./apps/future.md)

## Components

- [Component Docs Home](./comps/index.md)
- [finapp-mou-comp](./comps/finapp-mou-comp.md)
- [finapp-cashflow-comp](./comps/finapp-cashflow-comp.md)
- [pinapp-ftree-comp](./comps/pinapp-ftree-comp.md)

## CLI and ETL

- [second-brain-scripts and finapp-etl](./cli/second-brain-scripts.md)

## Database Schemas

- [Schema Overview](./schema/overview.md)
- [people schema](./schema/pinapp-people.md)
- [property schema](./schema/pinapp-property.md)
- [finapp_data schema](./schema/finapp-core.md)
- [finapp_summaries / views / journal](./schema/finapp-summaries.md)
- [finapp_benchmarks schema](./schema/finapp-benchmarks.md)
